...
app.post('/api/messages/:convoId/send', authMW, async (req, res) => {
  try {
    ...
    if (other) {
      await Notification.create({ recipient: other, sender: req.user._id, type: 'system',
        title: 'New message from ' + req.user.firstName, message: (content || 'Sent a photo').substring(0, 80) });
    }
    res.json({ conversation: convo });
