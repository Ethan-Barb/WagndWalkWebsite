...
exports.sendMessage = async (req, res, next) => {
  try {
    const { userId, title, message, sendSms, sendEmail } = req.body;
    ...
    if (sendSms && user.phone) {
      await sms.sendCustomSMS(user.phone, `Wag & Walk: ${title} — ${message}`).catch(() => {});
    }

    // Optionally send email
    if (sendEmail && user.email) {
      await emailService.sendPaymentConfirmation(user.email, 0).catch(() => {}); // reuse as generic
    }

    res.json({ notification, message: 'Message sent successfully' });
  } catch (err) { next(err); }
};
...
exports.sendBulkMessage = async (req, res, next) => {
  try {
    const { role, title, message, sendSms } = req.body;
    ...
    if (sendSms) {
      for (const u of users) {
        if (u.phone) sms.sendCustomSMS(u.phone, `Wag & Walk: ${title} — ${message}`).catch(() => {});
      }
    }
...
exports.updateSiteContent = async (req, res, next) => {
  try {
    let content = await SiteContent.getContent();
    const allowed = ['hero', 'announcement', 'testimonials', 'pricing', 'featuredWalkers', 'gallery', 'stats', 'aboutUs', 'emailCapture'];
    ...
    await content.save();
    // Re-fetch with populate so response has full walker data
    content = await SiteContent.findOne({ key: 'main' }).populate('featuredWalkers', 'firstName lastName profilePhoto walkerProfile');
    res.json({ content });
  } catch (err) { next(err); }
};

/* ── CMS: Upload image URL ────────────────────── */
exports.addGalleryImage = async (req, res, next) => {
  try {
    const { url, caption, altText } = req.body;
    if (!url) return res.status(400).json({ error: 'Image URL is required' });
    const content = await SiteContent.getContent();
    content.gallery.push({ url, caption: caption || '', altText: altText || '', order: content.gallery.length });
    await content.save();
    res.json({ gallery: content.gallery });
  } catch (err) { next(err); }
};

/* ── CMS: Remove gallery image ────────────────── */
exports.removeGalleryImage = async (req, res, next) => {
  try {
    const content = await SiteContent.getContent();
    content.gallery = content.gallery.filter(img => img._id.toString() !== req.params.imageId);
    await content.save();
    res.json({ gallery: content.gallery });
  } catch (err) { next(err); }
};

/* ── CMS: Add/update testimonial ──────────────── */
exports.upsertTestimonial = async (req, res, next) => {
  try {
    const content = await SiteContent.getContent();
    const { _id, clientName, dogName, text, rating, photoUrl, isVisible } = req.body;
    if (_id) {
      const t = content.testimonials.id(_id);
      if (t) { Object.assign(t, { clientName, dogName, text, rating, photoUrl, isVisible }); }
    } else {
      content.testimonials.push({ clientName, dogName, text, rating: rating || 5, photoUrl, isVisible: true, order: content.testimonials.length });
    }
    await content.save();
    res.json({ testimonials: content.testimonials });
  } catch (err) { next(err); }
};

/* ── CMS: Remove testimonial ──────────────────── */
exports.removeTestimonial = async (req, res, next) => {
  try {
    const content = await SiteContent.getContent();
    content.testimonials = content.testimonials.filter(t => t._id.toString() !== req.params.testimonialId);
    await content.save();
    res.json({ testimonials: content.testimonials });
  } catch (err) { next(err); }
};
