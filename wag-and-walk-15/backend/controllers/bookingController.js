  ...
  try {
    ...
    if (walkerId) {
      ...
      if (walker) {
        ...
        sms.sendBookingRequestSMS(walker.phone, booking).catch(() => {});
      }
    }
...
exports.acceptBooking = async (req, res, next) => {
  try {
    ...
    if (client) {
      notifications.sendBookingAccepted(client.email, booking).catch(() => {});
      sms.sendBookingAcceptedSMS(client.phone, booking).catch(() => {});
    }
...
exports.completeBooking = async (req, res, next) => {
  try {
    ...
    const client = await User.findById(booking.client);
    if (client) {
      notifications.sendBookingCompleted(client.email, booking).catch(() => {});
      sms.sendBookingCompletedSMS(client.phone).catch(() => {});
    }
...
exports.tipWalker = async (req, res, next) => {
  try {
    ...
    if (walker) {
      notifications.sendTipReceived(walker.email, amount).catch(() => {});
      sms.sendTipReceivedSMS(walker.phone, amount).catch(() => {});
    }
