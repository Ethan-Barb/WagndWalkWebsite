...
async function confirmBooking() {
  ...
  try {
    ...
    const body = {
      ...
      durationMinutes:    60,
      pickupAddress:      me.user.address || { street: '—', city: 'Naperville', state: 'IL' },
      specialInstructions: document.getElementById('book-notes')?.value || '',
    };

    await api('/bookings', { method: 'POST', body: JSON.stringify(body) });

    showAlert('book-alert','🎉 Walk booked successfully! Your walker will confirm shortly.', 'success');
    _selectedDogs  = []; _selectedDate = null; _selectedSlot = null; _selectedWalker = null;
    updateSummary();
    setTimeout(()=>document.querySelector('[data-section="bookings"]')?.click(), 1500);
